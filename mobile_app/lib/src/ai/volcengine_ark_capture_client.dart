...
class VolcengineArkCaptureClient {
  ...
  Stream<ArkCaptureStreamEvent> captureTextStream({
    ...
    if (response.statusCode < 200 || response.statusCode >= 300) {
      ...
      throw VolcengineArkCaptureException('火山方舟文本流式整理失败：HTTP ${response.statusCode} $body');
    }
  ...
  Map<String, dynamic> _requestBody({
    ...
    return {
      'model': model,
      'messages': [
        {'role': 'system', 'content': stream ? _streamSystemPrompt : _systemPrompt},
        {
          'role': 'user',
          'content': jsonEncode({
            'text': text,
...
class _ArkStreamParser {
  ...
  Iterable<ArkCaptureStreamEvent> add(String content) sync* {
    ...
    if (separatorIndex >= 0) {
      ...
      _jsonText += _pendingVisible.substring(separatorIndex + _captureJsonSeparator.length);
      _pendingVisible = '';
      _readingJson = true;
      return;
    }

    final safeLength = _pendingVisible.length - (_captureJsonSeparator.length - 1);
    if (safeLength > 0) {
...
把用户输入分为 todo、idea 或 unclear。todo 缺少时间、地点、主题时不要保存，missing_fields 写缺失字段，并提出一个自然中文追问。idea 不强制要求时间地点，应整理成简洁标题、摘要和标签。时间字段使用 ISO-8601 字符串；无法确定时填 null。会议默认 60 分钟，普通事项默认 30 分钟。
输出字段必须是 intent_type, confidence, title, summary, missing_fields, follow_up_question, should_save, todo_payload, idea_payload。todo_payload 必须包含 start_at, end_at, location, topic, reminder_at, status。idea_payload 必须包含 summary, source_hint, tags。todo_payload 和 idea_payload 二选一，另一个填 null。''';
...
如果 is_follow_up 为 true，说明用户正在回答上一轮追问。请把 text 作为补充信息合并进 pending_draft，尽量保留上一轮已确定字段，只更新缺失或被用户明确修正的字段，不要把补充回答当成全新记录。
把用户输入分为 todo、idea 或 unclear。todo 缺少时间、地点、主题时不要保存，missing_fields 写缺失字段，并提出一个自然中文追问。idea 不强制要求时间地点，应整理成简洁标题、摘要和标签。时间字段使用 ISO-8601 字符串；无法确定时填 null。会议默认 60 分钟，普通事项默认 30 分钟。
JSON 字段必须是 intent_type, confidence, title, summary, missing_fields, follow_up_question, should_save, todo_payload, idea_payload。todo_payload 必须包含 start_at, end_at, location, topic, reminder_at, status。idea_payload 必须包含 summary, source_hint, tags。todo_payload 和 idea_payload 二选一，另一个填 null。''';
