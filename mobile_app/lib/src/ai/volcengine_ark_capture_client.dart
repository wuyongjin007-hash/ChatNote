...
输出字段必须是 intent_type, confidence, title, summary, missing_fields, follow_up_question, should_save, todo_payload, todo_payloads, idea_payload, todo_delete_payload。todo_payload 必须包含 start_at, end_at, location, topic, reminder_at, status。todo_payloads 用于一句话包含多条待办的场景，每一项必须包含 title, summary, start_at, end_at, location, topic, reminder_at, status；单条待办时可以只返回 todo_payload，并让 todo_payloads 为空数组。idea_payload 必须包含 summary, source_hint, tags。todo_delete_payload 必须包含 operation, date_from, date_to, time_from, time_to, keyword；operation 只能是 delete 或 clear，日期范围使用左闭右开，时间范围用于重叠匹配。“删除提醒”表示删除整条待办。todo、idea、todoDelete 三种 payload 只保留对应的一种，其余填 null。''';
...
如果 is_follow_up 为 true，说明用户正在回答上一轮追问。请把 text 作为补充信息合并进 pending_draft，尽量保留上一轮已确定字段，只更新缺失或被用户明确修正的字段，不要把补充回答当成全新记录。
把用户输入分为 todo、idea、todoDelete 或 unclear。“删除、清空、取消某条或某段时间的待办/提醒”必须归类为 todoDelete，而不是新增 todo。todoDelete 不直接执行删除，should_save 固定为 false，并提取本地匹配条件。todo 缺少时间、地点、主题时不要保存，missing_fields 写缺失字段，并提出一个自然中文追问。idea 不强制要求时间地点，应整理成简洁标题、摘要和标签。时间字段使用 ISO-8601 字符串；无法确定时填 null。会议默认 60 分钟，普通事项默认 30 分钟。
JSON 字段必须是 intent_type, confidence, title, summary, missing_fields, follow_up_question, should_save, todo_payload, todo_payloads, idea_payload, todo_delete_payload。todo_payload 必须包含 start_at, end_at, location, topic, reminder_at, status。todo_payloads 用于一句话包含多条待办的场景，每一项必须包含 title, summary, start_at, end_at, location, topic, reminder_at, status；单条待办时可以只返回 todo_payload，并让 todo_payloads 为空数组。idea_payload 必须包含 summary, source_hint, tags。todo_delete_payload 必须包含 operation, date_from, date_to, time_from, time_to, keyword；operation 只能是 delete 或 clear，日期范围使用左闭右开，时间范围用于重叠匹配。“删除提醒”表示删除整条待办。todo、idea、todoDelete 三种 payload 只保留对应的一种，其余填 null。''';
